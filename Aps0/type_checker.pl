...
% les outils suivants ont ete ecrit avec l'aide de chatgpt
% outil de debug pour afficher le resultat du typage d'un programme APS0 (void si tout va bien)
print_list([]).
print_list([Head|Tail]) :-
    writeln(Head),
    print_list(Tail).

% outil permettant de recolter les types des arguments d'une fonction dans une liste
recolt_args_lists([], []).
...
test_prog :- 
    gamma_0(Gamma),
    type_prog(Gamma,prog([echo(id(x))]),void).
