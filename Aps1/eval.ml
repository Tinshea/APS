...
let rec eval_expr expr env mem =
  ...
  | ASTApp(e, es) -> (
        match e with
          ...
          | _ -> ( match eval_expr e env mem with
            ...
            | _ -> failwith "Application d'une non-fonction"
        )
      )
...
and eval_def def env mem =
  match def with
    ...
    | ASTProc(name, args, b) -> ((name, InP(b, id_of_args args, env))::env, mem)
    | ASTProcRec(name, args, b) -> ((name, InPR(b, name, id_of_args args, env))::env, mem)
