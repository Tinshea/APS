...
% alloc
type_expr(G, alloc(Expr), vec(_)) :-
    type_expr(G, Expr, int).

%nth
type_expr(G, nth(Expr1, Expr2), T) :-
    type_expr(G, Expr1, vec(T)),
    type_expr(G, Expr2, int).  

%len
type_expr(G, len(Expr), int) :-
    type_expr(G, Expr, vec(_)).

 %vset
type_expr(G, vset(Expr1, Expr2, Expr3), vec(T)) :-
    type_expr(G, Expr1, vec(T)),
    type_expr(G, Expr2, int),
    type_expr(G, Expr3, T).
...
type_stat(G, set(Lval, Expr), void) :-
    type_expr(G, Lval, Type),
    type_expr(G, Expr, Type).   
