...
(*on peut aussi passer la memoire en parametre*)
let rec add_to_env args values env =
  match args, values with
    [], [] -> []
    | a::tl1, v::tl2 -> (a, v)::(add_to_env tl1 tl2 env)
    | _ -> failwith "Arguments and values have different lengths"
;;
...
let rec eval_expr expr env mem =
  ...
  | ASTApp(e, es) -> (
        match e with
          ...
          | _ -> (let v = eval_expr e env mem in
                  let vs = eval_exprs es env mem in
                  match v with
                    | InF(e, a, envf) -> let new_env = add_to_env a vs envf in eval_expr e new_env mem
                    | InFR(e, name, a, envf) -> let new_env = (name, InFR(e, name, a, envf))::(add_to_env a vs envf) in eval_expr e new_env mem
                    | _ -> failwith "Unknown function"
          )
        )
    | ASTAlloc e -> (*A FAIRE*)
    | ASTLen e -> (*A FAIRE*)
    | ASTNth(e1, e2) -> (*A FAIRE*)
    | ASTVset(e1, e2, e3) -> (*A FAIRE*)
and eval_exprs es env mem =
  match es with
    ASTExpr e -> (eval_expr e env mem)::[]
    | ASTExprs(e, es) -> (eval_expr e env mem)::(eval_exprs es env mem)

let rec eval_lval lv env mem =
  match lv with
    ASTLId x -> List.assoc x env (*pas sur*)
    | ASTLNth(l,e) -> (*A FAIRE*)
